...
story_option = st.selectbox("Which Story?", [None, "Base", "Story1", "Story2"])
...
if st.button("Add to List"):
    ...
    if predecessor_map[selection] not in st.session_state.nodes_subset and SEQUENCE_FLAG:
    ...
    st.write(
        f"Completion Percentage: {(len(st.session_state.nodes_subset)-1)/(len(construction_graph.nodes())-1)*100}%"
    )
